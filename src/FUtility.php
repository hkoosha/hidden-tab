...
final class FUtility {
   ...
   * @param bool $allow_multi
   *   Whether if selecting multiple options is allowed.
   *
   * @return \Drupal\Core\Field\BaseFieldDefinition
   *   Defined field.
   */
  public static function list(string $title,
                              ?string $desc = NULL,
                              array $allowed = NULL,
                              bool $allow_multi = FALSE): BaseFieldDefinition {
    if ($allowed === NULL) {
      throw new \RuntimeException('illegal state');
    }
  ...
  public static function refrencerEntityRowBuilderForEntityList(RefrencerEntityInterface $entity,
    ...
    try {
      ...
      try {
        ...
        Utility::log($error0, $type, 'id');
        try {
          $row['id'] = $entity->id();
        }
        catch (\Throwable $error1) {
          Utility::log($error1, $type, 'id');
          Utility::say($error1);
          $row['id'] = t('Error');
        }
      }
      ...
      if ($entity instanceof StatusedEntityInterface) {
        try {
          ...
          Utility::log($error0, $type, 'status');
      ...
      if ($entity instanceof RefrencerEntityInterface) {
        ...
        try {
          ...
          Utility::log($error0, $type, 'target_hidden_tab_page');
          try {
            $row['target_hidden_tab_page'] = $entity->targetPageId();
          }
          catch (\Throwable $error1) {
            Utility::log($error1, $type, 'target_hidden_tab_page');
        ...
        try {
          ...
          Utility::log($error0, $type, 'target_user');
          try {
            $row['target_user'] = $entity->targetUserId();
          }
          catch (\Throwable $error1) {
            Utility::log($error1, $type, 'target_user');
        ...
        try {
          ...
          Utility::log($error0, $type, 'target_entity');
        ...
        try {
          ...
          Utility::log($error0, $type, 'target_entity_type');
        ...
        try {
          ...
          Utility::log($error0, $type, 'target_entity_bundle');
      ...
      Utility::log($error_x, $type, '~');
      if (!isset($row['id'])) {
        try {
          $row['id'] = $entity->id();
        }
        catch (\Throwable $error_y) {
          Utility::log($error_y, $type, 'id');
          Utility::say($error_y);
          $row['id'] = t('Error');
        }
      }
  ...
  public static function errorTargetsAlready(array $find): array {
    if (count($find) > 0) {
      ...
      try {
        ...
        Utility::log($error, $already->getEntityTypeId(), 'link', $already->id());
        $link = $already->id();
      }
      $top = [
        '@link' => $link,
  ...
  public static function loadTargets(FormStateInterface $form_state,
    ...
    try {
      ...
      if ($page_ && gettype($page_) !== 'string' && gettype($page_) !== 'integer') {
        $logger
          ->error('bad page type: {type}', ['type' => gettype($page_)]);
    ...
    try {
      ...
      if ($user_ && (gettype($user_) !== 'string' && gettype($user_) !== 'integer')) {
        $logger
          ->error('bad user type: {type}', ['type' => gettype($user_)]);
    ...
    try {
      ...
      if ($target_entity_ && (gettype($target_entity_) !== 'string' && gettype($target_entity_) !== 'integer')) {
        $logger
          ->error('bad target_entity type: {type}', ['type' => gettype($target_entity_)]);
  ...
  public static function entityCreationValidateTargets(FormStateInterface $form_state,
    ...
    try {
      ...
      $ok = FALSE;
      Utility::sayError($error);
      \Drupal::logger('hidden_tab')
        ->error('Error while validating msg={msg} trace={trace}', [
