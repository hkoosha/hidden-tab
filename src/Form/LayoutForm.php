...
class LayoutForm extends EntityForm {
   ...
   * {@inheritdoc}
   */
  public function __construct(Request $request, EntityStorageInterface $placementStorage) {
  ...
  private function table(array $regions): array {
    $blocks = [];
    $entities = Utility::placementsOfPage($this->placementStorage, $this->entity->id());
    ...
    foreach ($entities as $placement) {
      $blocks[$placement->region()][$placement->id()] = [
        ...
        'komponent_type' => HiddenTabKomponentPluginManager::labelOfPlugin($placement->komponentType()),
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    foreach ($form_state->getValue('blocks') as $placement => $attr) {
      ...
      $c = $this->placementStorage->load($placement);
