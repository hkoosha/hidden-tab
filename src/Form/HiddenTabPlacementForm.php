...
class HiddenTabPlacementForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state, $save_redirect = NULL): array {
    ...
    elseif (!HiddenTabTemplatePluginManager::man()
      ->exists($this->entity->targetPageEntity()->template())) {
    ...
    $form['region'] = [
      ...
      '#options' => HiddenTabTemplatePluginManager::regionsOfTemplate($template),
      '#disabled' => TRUE,
    ];
    ...
    $form['komponent_type'] = [
      ...
      '#options' => HiddenTabKomponentPluginManager::man()
        ->pluginsForSelectElement(),
      '#disabled' => TRUE,
    ];

    if (HiddenTabKomponentPluginManager::man()
      ->exists($this->entity->komponentType())) {
      $form['komponent'] = [
        '#type' => 'select',
        '#title' => $this->t('Komponent'),
        '#default_value' => $this->entity->komponent(),
        '#options' => HiddenTabKomponentPluginManager::komponentsOfPlugin($this->entity->komponentType()),
      ];
    }
    else {
      $form['komponent'] = [
    ...
    $form['extra']['target_entity_bundle'] = [
      '#type' => 'select',
      '#options' => FUtility::nodeBundlesSelectList(),
      '#required' => TRUE,
      '#default_value' => '~any~',
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!HiddenTabKomponentPluginManager::man()
      ->exists($form_state->getValue('komponent_type'))) {
